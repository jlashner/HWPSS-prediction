...
class HWP:
    ...
    def getHWPSS(self, freq, stokes, reflected = False, fit = False):
        """
            ...
            If fit is true, returns fit from  
        """
        if not fit:
            I = stokes[0]
            Q = stokes[1]
            ...
            if not reflected:
                print (stokes[0]-stokes[1]) * np.interp(freq, self.freqs, self.A4upT)
                A4 = stokes[1] * np.interp(freq, self.freqs, self.A4ppT) + (stokes[0]-stokes[1]) * np.interp(freq, self.freqs, self.A4upT)
                A2 = stokes[1] * np.interp(freq, self.freqs, self.A2upT) + (stokes[0]-stokes[1]) * np.interp(freq, self.freqs, self.A2upT)
            else:
                A4 = stokes[1] * np.interp(freq, self.freqs, self.A4ppR) + (stokes[0]-stokes[1]) * np.interp(freq, self.freqs, self.A4upR)
                A2 = stokes[1] * np.interp(freq, self.freqs, self.A2upR) + (stokes[0]-stokes[1]) * np.interp(freq, self.freqs, self.A2upR)
